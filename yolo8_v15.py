...
vehicle_codes = {area: {} for area in permanencia_areas}

# Adiciona validação para inicializar os dicionários se não existirem durante a execução
for area in permanencia_areas:
    if area not in permanencia_timestamps:
        permanencia_timestamps[area] = {}
    if area not in last_seen_times:
        last_seen_times[area] = {}
    if area not in vehicle_codes:
        vehicle_codes[area] = {}
            ...
            if tracks[0].boxes.id is not None and tracks[0].boxes.xyxy is not None and tracks[0].boxes.cls is not None:
                for box, track_id_tensor, class_id_tensor in zip(tracks[0].boxes.xyxy.cpu(), tracks[0].boxes.id.cpu(), tracks[0].boxes.cls.cpu()):
                    ...
                    if polygon_area.contains(centro):
                        ...
                        # Verifica se o track_id está registrado antes de calcular o tempo de permanência
                        if track_id in permanencia_timestamps[area_name]:
                            ...
                            print(f"DEBUG: Track ID={track_id} não encontrado em permanencia_timestamps na área {area_name}. Ignorando.")
                ...
                for track_id in list(permanencia_timestamps[area_name].keys()):
                    if track_id in last_seen_times[area_name]:  # Verifica se o track_id está presente em last_seen_times
                        ultima_vez_visto = last_seen_times[area_name][track_id]
                        ...
                        print(f"DEBUG: Track ID={track_id} não encontrado em last_seen_times na área {area_name}. Ignorando.")
