...
rem Execução App
cd %app_dir%
python.exe yolo8_v15.py --video_path %url% --model_path %app_dir%\modelo_mf_imgsz1280.pt --config_path %app_dir%\config\%camera%_config.json --area_config_path %app_dir%\area\%camera%_area.json --db_path %app_dir%\yolo8.db --output_dir %video_dir%\teste_rafa\%camera% --save_video true --video_interval 30 --output_width 640 --output_height 358 --permanencia_config_path %app_dir%\area\%camera%_area_tp.json
